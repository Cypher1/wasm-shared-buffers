...
impl HostContext<'_> {
    fn new(hunter_path: &str, runner_path: &str) -> Self {
        ...
        fork_container(hunter_path, HUNTER_SIGNAL_INDEX);
        fork_container(runner_path, RUNNER_SIGNAL_INDEX);
...
fn fork_container(module: &str, signal_index: usize) {
    match fork() {
        Ok(Fork::Parent(_)) => (),
        Ok(Fork::Child) => {
            let err = exec::execvp("gtk-rust-host/target/debug/container", &["container", module, &signal_index.to_string()]);
